...
function TurretWeapon:init(unit)
	...
	--TODO: AI Turret Support managers.groupai:state():register_usable_turret(self)
end
...
function TurretWeapon:_increase_heat()
	...
	local overheat_delta = self._overheat_speed * managers.player:upgrade_value("player", self._overheat_skill_id, 1)
...
function TurretWeapon:trigger_held(blanks, expend_ammo, shoot_player, target_unit, damage_multiplier)
	...
	if not self._lock_fire and self._next_fire_allowed <= Application:time() then
		...
		if fired then
			self._unit:base():send_action("fired")
...
function TurretWeapon:_upd_puppet_movement()
	...
	local spin_max = deflection --self._unit:movement()._spin_max or deflection
...
function TurretWeapon:on_player_enter()
	self._unit:base():send_action("player_enter")
...
function TurretWeapon:on_player_exit()
	self._unit:base():send_action("player_exit")
...
function TurretWeapon:set_active(state)
	...
	if state then
		self._unit:base():send_action("activate")
	else
		self._unit:base():send_action("deactivate")
	end
...
function TurretWeapon:lock_fire(lock)
	if lock then
		self._unit:base():send_action("lock_fire")
	else
		self._unit:base():send_action("unlock_fire")
	end
