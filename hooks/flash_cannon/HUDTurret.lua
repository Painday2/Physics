...
HUDTurret.DISMOUNT_PROMPT_TEXT_FONT = tweak_data.menu.pd2_medium_font
HUDTurret.DISMOUNT_PROMPT_TEXT_FONT_SIZE = tweak_data.menu.pd2_medium_font_size
HUDTurret.DISMOUNT_PROMPT_INITIAL_DELAY = 1
HUDTurret.DISMOUNT_PROMPT_HOLD_TIME = 4
HUDTurret.DISMOUNT_PROMPT_DELAY = 8
HUDTurret.DEFAULT_RETICLE = "weapons_reticles_smg_thompson"
...
function HUDTurret:show(turret_unit, bullet_type)
	...
	--TODO: add this back fuck managers.hud:unselect_all_weapons()
	--TODO: add this back fuck managers.queued_tasks:queue("hud_turret_prompt", self.show_prompt, self, nil, HUDTurret.DISMOUNT_PROMPT_INITIAL_DELAY, nil)
...
function HUDTurret:hide()
	...
	-- self._heat_indicator_background:stop()
	-- self._heat_indicator_background:animate(callback(self, self, "_animate_hide"))

	-- self._heat_indicator_foreground:stop()
	-- self._heat_indicator_foreground:animate(callback(self, self, "_animate_hide"))

	-- self._shell_bg:stop()
	-- self._shell_bg:animate(callback(self, self, "_animate_hide"))

	-- self._shell_fade:stop()
	-- self._shell_fade:animate(callback(self, self, "_animate_hide"))

	-- self._shell:stop()
	-- self._shell:animate(callback(self, self, "_animate_hide"))

	--TODO: add this back fuck managers.queued_tasks:unqueue("hud_turret_prompt")
	-- self._reticle:stop()
...
function HUDTurret:show_prompt()
	...
	--TODO: add this back fuck managers.queued_tasks:queue("hud_turret_prompt", self.hide_prompt, self, nil, HUDTurret.DISMOUNT_PROMPT_HOLD_TIME, nil)
end

function HUDTurret:hide_prompt()
	self._dismount_prompt_panel:stop()
	self._dismount_prompt_panel:animate(callback(self, self, "_animate_hide"))
	--TODO: add this back fuck managers.queued_tasks:queue("hud_turret_prompt", self.show_prompt, self, nil, HUDTurret.DISMOUNT_PROMPT_DELAY, nil)
end
...
function HUDTurret:_animate_show(object, animation_duration)
	...
	while duration > t do
		...
		t = t + dt
		local current_alpha = Easing.inout_quart(0, 1, t/duration)
...
function HUDTurret:_animate_hide(object, animation_duration)
	...
	while t < duration do
		local dt = coroutine.yield()
		t = t + dt
		local current_alpha = Easing.inout_quart(1, -1, t/duration)
...
function HUDTurret:_animate_shell_insert()
	...
	while t < duration do
		local dt = coroutine.yield()
		t = t + dt
		local alpha = Easing.in_quad(0, 1, t/duration)
...
function HUDTurret:_animate_shell_fire()
	...
	while t < duration do
		...
		if new_pos > self._shell_bg:x() + shell_width then
			local alpha_alpha = Easing.in_quad(0, 1, t/duration)
