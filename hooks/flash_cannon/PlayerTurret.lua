...
function PlayerTurret:update(t, dt)
	...
	if self._turret_sync_dt > 0.2 then
		self._turret_unit:base():send_rotation()
...
function PlayerTurret:_announce_cooldown()
	...
	--TODO: add this back fuck managers.queued_tasks:unqueue_all(self._announce_shooting_clbk_id, self)
	--TODO: add this back fuck managers.queued_tasks:queue(self._announce_shooting_clbk_id, self._enable_announcing, self, nil, final_cooldown)
end
...
function PlayerTurret:_position_player()
...
function PlayerTurret:_reposition_player()
	if not managers.job:current_level_id() then
		return
	end
